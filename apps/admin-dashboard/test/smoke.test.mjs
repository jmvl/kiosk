...
describe('@retail-kiosk/admin-dashboard v1', () => {
  ...
  it('ships the React scheduler and telemetry UI against local APIs', () => {
    ...
    assert.match(source, /<CentralSchedulesDeploymentsPanel central=\{central\} baseUrl=\{centralApiBaseUrl\} onRefresh=\{refreshCentral\} \/>/);
    assert.match(source, /VITE_CENTRAL_API_BASE_URL/);
    assert.match(source, /\/v1\/admin\/fleet\/overview/);
    assert.match(source, /\/v1\/admin\/kiosks/);
    assert.match(source, /\/v1\/admin\/schedules/);
    assert.match(source, /\/v1\/admin\/deployments/);
    assert.match(source, /Central back office/);
    assert.match(source, /Local kiosk tools/);
    assert.match(source, /Schedules & deployments/);
    assert.match(bundle, /Central back office/);
    assert.match(bundle, /Local kiosk tools/);
    assert.match(bundle, /Schedules & deployments/);
    assert.match(bundle, /VITE_CENTRAL_API_BASE_URL/);
    assert.match(bundle, /\/v1\/admin\/fleet\/overview/);
    assert.match(bundle, /\/v1\/admin\/kiosks/);
    assert.match(bundle, /\/v1\/admin\/schedules/);
    assert.match(bundle, /\/v1\/admin\/deployments/);
    assert.match(bundle, /No central schedules returned/);
    assert.match(bundle, /No central deployments returned/);
    assert.match(bundle, /central API not configured/);
