...
if [ "$RESTART_SERVER" = "1" ]; then
  ...
  pkill -u "$(id -un)" -f "kiosk-coin-slot-monitor.py .*${COIN_SERIAL}" 2>/dev/null || true
  for _ in $(seq 1 40); do
    if ! pgrep -u "$(id -un)" -f "ptit_lion_server.py .*--port ${PORT}|server.py .*--port ${PORT}|kiosk-coin-slot-monitor.py .*${COIN_SERIAL}" >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
